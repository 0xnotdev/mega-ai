...
async def get_mock_chunks(query: str) -> list[dict]:
    """Returns mock chunks when DB is empty. Replace with pgvector query later."""
    return [
        {"chunk_id": "chunk_001", "content": f"Source A discusses {query[:30]}: key facts about topic overview and background."},
        {"chunk_id": "chunk_002", "content": f"Source B elaborates on {query[:30]}: specific details, statistics, and expert analysis."},
    ]
...
async def run_retrieval(query: str) -> RetrievalResult:
    ...
    for c in raw_citations:
        if isinstance(c, dict):
            citations.append(ChunkCitation(
                ...
                content_snippet=str(c.get("content_snippet", c.get("content", ""))),
                relevance_score=float(c.get("relevance_score", 1.0)),
                contributed_to=str(c.get("contributed_to", query)),
            ))
    ...
    return RetrievalResult(
        ...
        citations=citations,
        hops_performed=data.get("hops_performed", len(chunks)),
        chunks_consulted=[c["chunk_id"] for c in chunks]
    )